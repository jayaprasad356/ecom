...
if (isset($_GET['table']) && $_GET['table'] == 'orders') {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        ...
        if (!empty($_GET['start_date']) && !empty($_GET['end_date'])) {
            ...
            $where .= " where (name like '%" . $search . "%' OR o.id like '%" . $search . "%' OR o.mobile like '%" . $search . "%' OR address like '%" . $search . "%' OR `payment_method` like '%" . $search . "%' OR `delivery_charge` like '%" . $search . "%' OR `delivery_time` like '%" . $search . "%' OR o.`status` like '%" . $search . "%' OR o.`date_added` like '%" . $search . "%')";
        }
    }
    ...
    $sql = "SELECT COUNT(distinct(o.id)) as total FROM `orders` o LEFT JOIN order_items oi ON o.id=oi.order_id LEFT JOIN users u ON u.id=o.user_id" . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "select o.*,u.name FROM orders o LEFT JOIN users u ON u.id=o.user_id  LEFT JOIN pincodes p ON p.id=o.pincode_id LEFT JOIN order_items oi ON o.id=oi.order_id " . $where . " GROUP BY o.id ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    // echo $sql;
    $db->sql($sql);
    $res = $db->getResult();
...
// data of 'bulkseller' table goes here
if (isset($_GET['table']) && $_GET['table'] == 'bulkseller') {
    ...
    foreach ($res as $row) {
        ...
        $operate = '<a href="edit-bulkseller.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
       // $operate .= ' <a href="edit-category.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
        //$operate .= ' <a class="btn-xs btn-danger" href="delete-category.php?id=' . $row['id'] . '"><i class="fa fa-trash-o"></i>Delete</a>';

        $tempRow['id'] = $row['id'];
        $tempRow['business_type'] = $row['business_type'];
        ...
        $tempRow['mobile'] = $row['mobile'];
        $tempRow['pan_card'] = "<a data-lightbox='category' href='upload/documents/" . $row['pan_card'] . "' data-caption='pancard'><img src='upload/documents/" . $row['pan_card'] . "' title='" . $row['name'] . "' height='50' /></a>";
